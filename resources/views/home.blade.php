
<!DOCTYPE html>
<!--HTML5 doctype huasyu-->
<html>

    <head>

        <title>@yield('title')</title>

        <meta http-equiv="Content-type" content="text/html; charset=utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=0, minimal-ui">
        <meta name="apple-mobile-web-app-capable" content="yes">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">

        <link rel="SHORTCUT ICON" href="" />
        <link rel="stylesheet" type="text/css" href="https://m.bebasbayar.com/assets/css/introjs.css">
        <link rel="stylesheet" type="text/css" href="https://m.bebasbayar.com/assets/css/icons.min.css">
        <link rel="stylesheet" type="text/css" href="https://m.bebasbayar.com/assets/css/font-awesome.min.css">
        <link rel="stylesheet" type="text/css" href="https://m.bebasbayar.com/assets/css/af.ui.bebasbayar.css">
        <link rel="stylesheet" type="text/css" href="https://m.bebasbayar.com/assets/css/af.ui.css">
        <link rel="stylesheet" type="text/css" href="https://m.bebasbayar.com/assets/css/bebasbayar.css">
        <link rel="stylesheet" type="text/css" href="https://m.bebasbayar.com/assets/css/private.css">

    
        <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/jquery.min.js"></script>
        <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/fastclick.js"></script>
        <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/appframework.ui.min.js"></script>
        <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/jquery.fileDownload.js"></script>
        <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/bebasbayar.js"></script>
        <!-- FB -->
        
</head>
<body class="bebasbayar light">
    <div style="display: none;" id="phplive_btn_1438671160"></div>

    <div class="view" id="bb-main">
        <header>
            <a class="menuButton" data-left-menu="sidebar-container" data-transition="cover">&nbsp;</a>
            <h1 style='padding-left: 10px;'><a href="/home" data-ignore="true">meliPAY</a></h1>
            <a id="open-help" href="javascript:void(0)" class='pull-right menu-header'>
                <img class="icon" src="https://m.bebasbayar.com/assets/img/icon/ic_bb_info.png" />
            </a>
            <a id="open-fave" href="javascript:void(0)" data-fave="" class='pull-right menu-header'>
                <img class="icon" src="https://m.bebasbayar.com/assets/img/icon/ic_star_border.png" />
            </a>
            <a id="open-walkthrough" href="javascript:show_walkthrough()" class='pull-right menu-header'>
                <i class="fa fa-question-circle-o" aria-hidden="true"></i>
            </a>
            <a id="open-mailbox" href="mailbox" class='pull-right menu-header'>
                <img class="icon" src="https://m.bebasbayar.com/assets/img/icon/ic_bb_pesan_member.png" />
            </a>
            <a id="open-livechat" href="javascript:void(0)" data-href="https://support.bebasbayar.com:4430/phplive/js/phplive_v2.js.php?v=0|1438671160|0|" class='pull-right menu-header'>
                <img class="icon" src="https://m.bebasbayar.com/assets/img/icon/ic_bb_kontak.png" />
            </a>
            <a id="open-search" href="javascript:void(0)" class='pull-right menu-header'>
                <img class="icon" src="https://m.bebasbayar.com/assets/img/icon/ic_menu_search.png" />
            </a>
        </header>

        
        <div class="pages">
            <link href="https://m.bebasbayar.com:443/assets/css/edukasi.css" rel="stylesheet" type="text/css" />
<style>
    .bb-product-desc{
        height: 68px;
    }
    .media-title2{
        color: #333333;
    }
    .carouselbox {
        font-family: helvetica,sans-serif;
        font-size: 14px;
        position: relative;
        /*border: 1px solid #ccc;*/
        /*box-shadow: 2px 2px 10px #ccc;*/
        overflow: hidden;
    }
    .carouselbox .content {
        margin: 0;
        padding: 0;
    }
    .carouselbox .content li {
        margin: 0;
        padding: 0;
        width: 100%;
        height: 100%;
        list-style: none;
    }
    .carouselbox .content .wrapper {
        /*padding: 0 30px;*/
    }
    .carouselbox .content .wrapper > .title {
        font-weight: bold;
    }
    .carouselbox .content .wrapper > .description {
        display: none;
    }
    .carouselbox .content .wrapper > .date {
        position: absolute;
        bottom: 0;
        font-size: 11px;
        color: green;
        font-style: italic;
    }
    .carouselbox .content .button {
        position: absolute;
        bottom: 0;
        right: 30px;
        padding: 5px 5px 0 5px;
    }
    .carouselbox.active {
        height: auto;
        min-height: 173px;
    }
    .carouselbox.active li {
        position: absolute;
        top: 200px;
    }
    .carouselbox.active li.current {
        top: 0;
    }
    .carouselbox .control {
        position: absolute;
        top: 45%;
    }
    .carouselbox .control.prev {
        left: 0;
    }
    .carouselbox .control.next {
        right: 0;
    }
    .percent-60 {
        width: 60% !important;
    }
    .percent-29 {
        width: 29% !important;
    }
    .widget-pulsa-title {
        letter-spacing: 0px !important;
        font-size: 12px !important;
        font-weight: 100px;
        margin: 0px;
        padding: 0px;
        height: 25px;
    }
    .widget-pulsa {
        margin: 0px !important;
        padding: 0px !important;
        border: none !important;
    }
    .widget-pulsa-input {
        padding: 2px 5px !important;
        width: 69% !important;
        height: 25px !important;
    }
    .widget-pulsa-btn {
        width: 25% !important;
    }
    .widget-pulsa-icon {
        width: 5% !important;
        padding: 3px;
    }
    .widget-pulsa-icon, .widget-pulsa-btn, .widget-pulsa-input {
        display: inline-block;
        vertical-align: middle;
    }
    .bebasbayar .panel .card {
        margin: 0px 0 0 0;
        padding: 15px;
    }
    .rmv-padding-top {
        padding-top: 2px !important;
        padding-bottom: 2px !important;
    }
    .button.white{
        background-color: #FDFDFD;
        color: #1976D2;
        font-weight: 400;
    }
    .bts-popup-container .bts-popup-close {
        position: absolute;
        top: 8px;
        right: 8px;
        width: 30px;
        height: 30px;
    }
    .bts-popup-container .bts-popup-close::before, .bts-popup-container .bts-popup-close::after {
        content: '';
        position: absolute;
        top: 12px;
        width: 16px;
        height: 3px;
        background-color: white;
    }
    .bts-popup-container .bts-popup-close::before {
        -webkit-transform: rotate(45deg);
        -moz-transform: rotate(45deg);
        -ms-transform: rotate(45deg);
        -o-transform: rotate(45deg);
        transform: rotate(45deg);
        left: 8px;
    }
    .widget-speedcash{
        position: relative;
        height: 60px;
    }
    .widget-speedcash-child{
        /*position: absolute;*/
        border-radius: 5px;
        height: 60px;
        min-width: 247px;
        width: 80%;
        margin: 0 auto;
        background-color: #FFFFFF;
        /*left:16%;*/
        /*margin-right: 16%;*/
        padding: 5px;
    }
    .topup{
        background-color: #EF6C00 !important;
        border: 1px solid #EF6C00;
        width: 80%;
        padding: 9px;
        margin-top: 6px;
        color: #FFFFFF;
    }
    .home_banner_top{
        height: 179px;
    }
</style>

<div class="panel active wrapper-scroll">
    <div class="" style='padding-bottom: 5px;'>
        <div class="grid">
            <div class="home_banner_top">

                <div class=''>
                    <div class="grid">

                        <div class="grid__item one-whole">
                            <div class="home-grid static carouselbox active text-left" >
                                <ol class="content">
                                    <li class="current">
                                        <div class="grid__item one-whole text-center mrg10T">
                                                                                            <img src="https://m.bebasbayar.com/assets/img/avatar/M-avatar.png" class="user-image" alt="User Image" style="width:80px;"/>
                                                                                    </div>
                                        <div class="grid__item one-whole text-center">
                                            <h4 style='margin: 0; padding: 0; height: auto;font-weight: normal;' class=" font-white font-size-12 mrg5B" id='sidebar-account__name'>
                                                {{ Auth::user()->name }}
                                            </h4>
                                            <div class="widget-speedcash">
                                                <div class="widget-speedcash-child">
                                                    <a href="https://m.bebasbayar.com:443/pages/speedcash">
                                                        <div style="width:70%;float:left">
                                                            <img style="margin-top: 6px;width: auto;height: 36px;float: left;padding-right: 15px" src="https://m.bebasbayar.com:443/assets/img/logosc/logo-sc.png">
                                                            <p style="color:#333333;text-align: center">
                                                                Saldo SpeedCash<br>
                                                                <span id='sidebar-account__balance' style="color:#333333;text-align: center;font-weight: bold">
                                                                    Rp&nbsp;5.666,00                                                                </span>
                                                            </p>
                                                        </div>
                                                    </a>
                                                    <div style="width:30%;float:left">
                                                        <button type="button" onclick="javascript:window.location.href = 'pages/deposit'" class="topup">TOPUP</button>
                                                    </div>
                                                </div>
                                            </div>
                                            <!--<small style="color:#a6daff">Premium User</small>-->
                                        </div>
                                    </li>
                                                                    </ol>
                                                                <i class="control prev material-icons font-white">&#xE314;</i>
                                <i class="control next material-icons font-white">&#xE315;</i>
                            </div>
                        </div>


                    </div>

                </div>
            </div>
            <div class="tab fixed" style="margin-top: -1px;">
                <a href="/" class="active" id="tab1" >
                    Beranda
                </a>
                <a href="pages/promo" id="tab2">
                    Promo
                </a>
            </div>
        </div>
        <div class="grid menu_utama">
            <div class="card rmv-padding-top" style="padding: 0;">
    <div class="card-header ch-alt">
        <h2 class="widget-pulsa-title" style="padding: 5px; line-height: 1.5;">Beli Pulsa dan Paket Data</h2>
        <form class="" role="form">
            <div class="input-group widget-pulsa">
                <div class="widget-pulsa-icon"><i class="fa fa-phone" style="font-size: 20px;"></i></div>
                <input type="text" class="form-control widget-pulsa-input" id="no_telp_pulsa" value="6283164306373" />
                <button class="button white percent-29 pull-right widget-pulsa-btn" id="beli_pulsa" style="padding: 0;">Lanjutkan</button> 
            </div>
        </form>
    </div>
</div>
<script>
    $(document).ready(function() {
        //transaksi pulsa
        $("#no_telp_pulsa").numericIt();
        $("#beli_pulsa").on("click", function() {
            var notelp = $("#no_telp_pulsa").val();
            if (notelp[0] == "6" && notelp[1] == "2")
                notelp = "0" + notelp.slice(2);
            window.open("pages/pulsa/#" + notelp, '_self');
            return false;
        });

    });
</script>            <div class="row" id="list-suplier"></div>
            <div class="grid__item one-quarter">
                <div data-href="pages/deposit" class="home-grid" >
                    <div class='home-grid-plate'>
                        <img class="img-home-deposit" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">
                    </div>
                    <div class='home-grid-title'>Deposit</div>
                </div>
            </div><!--
                                                                                                                                                                                                                            
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4022" data-href="pages/pulsa#paketdata" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-paket-data" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Paket Data Internet                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4022" data-href="pages/pulsa" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-pulsa" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Pulsa Reguler                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4017" data-href="pages/electric" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-pln" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Listrik                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4019" data-href="pages/airplane" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-tiket-pesawat" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Pesawat                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4003" data-href="pages/train" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-kereta-api" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Kereta Api                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4003" data-href="pages/train?utm_source=railink" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-railink" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Railink                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4011" data-href="pages/water" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-pdam" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Air                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4022" data-href="pages/vouchergame" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-game" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Voucher Game                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4029" data-href="pages/tvcable" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-tv-kabel" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                TV Kabel                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="103" data-href="penjualan/supplier" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-pembelian" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Belanja                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="6004" data-href="pages/penjualan_buku" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-edukasi" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Pembelian Buku                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4046" data-href="pages/bpjsks?p=ASRBPJSKS" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-bpjs" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                BPJS KS                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4025" data-href="pages/speedy" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-speeady" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Speedy                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4005" data-href="pages/creditcard" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-kartu-kredit" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Kartu Kredit                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4023" data-href="pages/phone" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-telkom" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Telepon/Flexi                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4001" data-href="pages/insurance" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-asuransi" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Asuransi                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="3011" data-href="pages/scti" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-transfer-saldo" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Transfer                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="3005" data-href="pages/cashout_bank" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-tarik-tunai" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Transfer ke Rekening Bank                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="1022" data-href="pages/calendar" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-autobayar" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Bayar Otomatis                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4027" data-href="pages/pulsapasca" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-pascabayar" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Pascabayar                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4007" data-href="pages/multifinance" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-cicilan" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Cicilan                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="1020" data-href="/mailbox" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-inbox" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Pesan Antar Member                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4051" data-href="pages/cashout_misc" class="home-grid menu-disabled" >
                            <div class='home-grid-plate'>
                                <img class="img-home-file" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Keuangan Lain Lain                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4047" data-href="pages/zakat" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="img-home-zakat" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>
                            </div>
                            <div class='home-grid-title'>
                                Zakat                            </div>
                        </div>
                    </div><!--
                                                                                                                                                                                                                                    
                    --><div class="grid__item one-quarter">
                        <div data-fitur="4014" data-href="pages/pajak" class="home-grid " >
                            <div class='home-grid-plate'>
                                <img class="" src="data:image/png;base64,"/>
                            </div>
                            <div class='home-grid-title'>
                                Bayar Pajak                            </div>
                        </div>
                    </div><!--
                                -->
        </div>
    </div>
    <div class="div_unduh">
        <a class="button button-unduh" href="javascript:void(0);" >
            <span class="download_app">
                <i class="material-icons mrg0A pull-left">file_download</i> &nbsp; Unduh Aplikasi &nbsp;             
            </span>
            <span class="close_unduh"><i class="material-icons">close</i></span>
        </a>        
    </div>
</div>
<script type="text/javascript">
    function popup(message) {
        $.afui.popup({
            id: 'pesan',
            title: 'Information',
            message: message,
            cancelText: "OK",
            cancelOnly: true
        });
//        $('#pesan .header').html('');
//        $('#pesan').html(message);

    }
    $(document).ready(function () {
        var cookie = getCookie("popupNews");
        if (cookie != 1) {
            document.cookie = "popupNews=1";
        }

        function getCookie(cname) {
            var name = cname + "=";
            var decodedCookie = decodeURIComponent(document.cookie);
            var ca = decodedCookie.split(';');
            for (var i = 0; i < ca.length; i++) {
                var c = ca[i];
                while (c.charAt(0) == ' ') {
                    c = c.substring(1);
                }
                if (c.indexOf(name) == 0) {
                    return c.substring(name.length, c.length);
                }
            }
            return "";
        }

        $('.popupnews').click(function () {
            var link = $('.popupnews').attr('title');
            window.open(link, '_blank');
        });

        $.getJSON("/edukasi/get_supplier_tag", {
            tag: "belanja"
        }, function (r) {
            if (!r.error) {
                var w = $("#list-suplier");
//                    $.each(r.data, function(i, v){
                var id = 'shop-' + r.data[0].nama.replace(/\s/g, '-'), g = '/penjualan/tipe/' + r.data[0].id + '-' + r.data[0].nama.replace(/\s/g, '-') + "?utm_source=bb_shop&utm_medium=tipe&utm_campaign=produk " + r.data[0].nama;
                $('<div>', {class: 'bb-shop grid'})
                        .append($('<h4>', {class: 'shop-title', style: 'color:#333333'}).text('Belanja Barang')
                                .append($('<br>'))
                                .append($('<p>', {class: 'blue shop-more pull-left', style: 'color:#878787'}).html('Temukan berbagai barang kebutuhan'))
                                .append($('<a>', {href: g, class: 'blue shop-more pull-right', title: 'Lihat semua barang di ' + r.data[0].nama, style: 'color: #1976D2;'}).html('LAINNYA'))
                                )
                        //                        .append($('<small>').text(v.keterangan))
                        .append($('<div>', {class: 'shop-content', id: id, 'data-type': r.data[0].id, 'data-view': r.data[0].view_type}))
                        .appendTo(w);
//                    });
            }
        }).done(function () {
            $('.bb-shop .shop-content').getBelanja();
            $.when.apply(null, aj).done(function (e) {
                console.log('done');
            });
        });
        var dep =
                '<div class="bts-popup" role="alert">' +
                '<div class="bts-popup-container">' +
                '<p>Deposit melalui transfer bank akan di tutup pada jam 21.00 WIB, deposit tetap dapat dilakukan melalui Mandiri 24 atau Virtual Acount BRI & BNI</p>' +
//                '<a href="#0" class="bts-popup-close img-replace"><i class="fa fa-window-close"></i></a>'+
                '</div>' +
                '</div>'
                ;
    });
    $(".home-grid:not(.static)").on("click", function () {
        var href = $(this).data("href");
        location.href = href;
    });
    $(".carouselbox .content img").on("click", function () {
        var title = $(this).siblings(".title_news").val();
        var images = '<img class="img-responsive mrg10B" src="' + $(this).attr("src") + '">';
        var description = $('<div>').append($(this).siblings(".desc_news").val()).html();
        reveal_content(title, "<div style='padding: 20px;'>" + images + " " + htmlEntityDecode(description) + "</div>");
    });
    (function () {
        var box = document.querySelector('.carouselbox');
        if (box != null) {
            var next = box.querySelector('.next');
            var prev = box.querySelector('.prev');
            var swipe = box.querySelector('ol li');
            var items = box.querySelectorAll('.content li');
            var counter = 0;
            var amount = items.length;
            var current = items[0];
            box.classList.add('active');
            function navigate(direction) {
                current.classList.remove('current');
                counter = counter + direction;
                if (direction === -1 &&
                        counter < 0) {
                    counter = amount - 1;
                }
                if (direction === 1 &&
                        !items[counter]) {
                    counter = 0;
                }
                current = items[counter];
                current.classList.add('current');
            }
            next.addEventListener('click', function (ev) {
                navigate(1);
            });
            prev.addEventListener('click', function (ev) {
                navigate(-1);
            });
//            swipe.addEventListener("swiperight",function(event){
//                navigate(1);
//                console.log("swipe")
//            })

            navigate(0);
        }

        if ($('.wrapper-scroll').length && $('.wrapper-scroll').find('.fixed').length) {
            $('.wrapper-scroll').scroll(function () {
                var s = $(this).scrollTop();
                if (s >= 155)
                    $('.fixed').css({position: 'fixed', top: '40px', 'z-index': '250'});
                else
                    $('.fixed').removeAttr('style').css('margin-top', '-1px');
//                console.log(s);
//                console.log("->"+(s-1));

                if (s == 0) {
                    $(".home_banner_top").css('transform', 'translateY(' + s + 'px)');
//                    $('.home_banner_top').removeAttr('style')
                } else {
                    var x = s - 15;
                    $(".home_banner_top").css('transform', 'translateY(' + x + 'px)');
                }
            });
        }
        $(".carouselbox ol li").on("swipe", function (event) {
//                console.log(event.target);
            navigate(1);
        });
        $(".download_app").on("click", function () {
            window.open('https://play.google.com/store/apps/details?id=com.bm.sc.bebasbayar');
        });
        var cookie = getCookie('close_download');
        $(".close_unduh").on("click", function () {
            $(".div_unduh").fadeOut();
            setCookie('close_download', "1", 90);
        });
        if (cookie == null || cookie == "") {

            $(".div_unduh").show();
        } else {
            $(".div_unduh").hide();
        }
    })();
    //set the cookie when they first hit the site
    function setCookie(c_name, value, exdays)
    {
        var exdate = new Date();
        exdate.setDate(exdate.getDate() + exdays);
        var c_value = escape(value) + ((exdays == null) ? "" : "; expires=" + exdate.toUTCString());
        document.cookie = c_name + "=" + c_value;
    }

//check for the cookie when user first arrives, if cookie doesn't exist call the intro.
    function getCookie(c_name)
    {
        var c_value = document.cookie;
        var c_start = c_value.indexOf(" " + c_name + "=");
        if (c_start == -1)
        {
            c_start = c_value.indexOf(c_name + "=");
        }
        if (c_start == -1)
        {
            c_value = null;
        } else
        {
            c_start = c_value.indexOf("=", c_start) + 1;
            var c_end = c_value.indexOf(";", c_start);
            if (c_end == -1)
            {
                c_end = c_value.length;
            }
            c_value = unescape(c_value.substring(c_start, c_end));
        }
        return c_value;
    }
</script> 
<script type="text/javascript" src="https://m.bebasbayar.com:443/assets/js/jquery.formatCurrency.js"></script>
<script type="text/javascript" src="https://m.bebasbayar.com:443/assets/js/jquery-ui.min.js"></script>
<!--<script type="text/javascript" src="https://m.bebasbayar.com:443/assets/js/hammer.min.js"></script>
<script type="text/javascript" src="https://m.bebasbayar.com:443/assets/js/hammer-time.min.js"></script>-->
<script type="text/javascript" src="https://m.bebasbayar.com:443/assets/js/penjualan_barang.js"></script>
        </div>

        
        <footer>
            <h5>BebasBayar &copy; 2014-2019, PT. Bimasakti Multi Sinergi <br> Powered by: SpeedCash</h5>
        </footer>
    </div>


    <div class="view" id="bb-modal">
        <header style="z-index: 2;">
            <h1 class='modal-title'></h1>
            <a href='javascript:void(0)' class='pull-right' onclick="$.afui.loadContent('#bb-main', false, false, 'fade');">
                <i class="material-icons white">&#xE5C9;</i>
            </a>
            <a href='javascript:void(0)' class='search pull-right' style="display: none;">
                <i class="material-icons white">&#xE8B6;</i>
            </a>
        </header>
        <div class="pages">
            <div class="panel active">
                <div class='modal-loader ui-loader'>
                    <img src="https://m.bebasbayar.com/assets/img/indicator.gif">
                    <br><br>
                    <h1>Mohon Tunggu</h1>
                </div>
                <div class="modal-content">

                </div>
            </div>
        </div>
    </div>
    <div id="afui">
        <div id="header">
        </div>

        <div id="content">
            <div id="favs" data-title="Favorites" class="panel" selected="true">
                <button onclick="full_screen();">Full Screen</button>
            </div>

            <div id="welcome" data-title="Welcome" class="panel" selected="true">
                <ul class="list">
                    <li><a class="icon star" href="#favs">Favorites <span class="af-badge">2</span></a></li>
                </ul>
            </div>    
        </div>

        <div id="navbar">
            <a href="#welcome" class="icon home">Home</a>
            <a href="#favs" class="icon star">Favorites<span class="af-badge">2</span></a>
        </div>

    </div>

    <nav id="sidebar-container">
        <div class="view active">
            <header style="height: 175px;color: #FFF;line-height: 1.5;">
                <a href="javascript:void(0)" class='pull-left' data-menu-close>
                    <i class="material-icons white" style="font-size: 30px;">chevron_left</i>
                </a>
                <!--                    <h1>
                                        <a href="javascript:void(0)" data-menu-close>
                                            Menu
                                        </a>
                                    </h1>-->
                <a href="javascript:void(0)" class='pull-right refresh_data' >
                    <i class="material-icons white" style="margin: 5px 10px;">refresh</i>
                </a>
                                <div class="one-whole text-center mrg10T">
                                            <img src="https://m.bebasbayar.com/assets/img/avatar/M-avatar.png" class="user-image" alt="User Image" style="width:80px;"/>
                                    </div>
                <div class="one-whole text-center">
                    <h4 style='margin: 0; padding: 0; height: auto;font-weight: normal;' class=" font-white font-size-12 mrg5B" id='sidebar-account__name'>
                        Candra Syahputra                    </h4>
                    <h4 style='margin: 0; padding: 0; height: auto;margin-bottom: 3px;position: relative;' class=" font-white">
                        <span  id='sidebar-account__balance'>Rp&nbsp;5.666,00</span> 
                    </h4>
                    <small style="color:#a6daff">Premium User</small>
                </div>
            </header>

            <div class="pages">
                <div class="panel active" style="padding: 0;">
                    
<ul class="sidebar-menu list" style="margin-bottom: 70px;">
    <li id="menu-home">
        <a href="/" class="menu-favorit">
            <img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Home.png"/>&nbsp;
            <span>Beranda</span>
        </a>
    </li>
    <li id="menu-favorit">
<!--        <a href="/favorit" class="menu-favorit">
            <img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Menu-Favorit.png"/>&nbsp;
            <span>Menu Favorit</span>
        </a>-->
        <a href="/favorit" class="menu-favorit">
            <img class="icon-bb-sidebar-menu-favorit img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;
            <span>Menu Favorit</span>
        </a>
    </li>
    
    <!--    <li id="Afiliasi">
        <a href="#">
            <img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Afiliasi.png"/>&nbsp;
            <span>Afiliasi</span>
            <i class="material-icons pull-right">&#xE5CC;</i>
        </a>
        <a href="#">
            <img class="icon-bb-sidebar-afiliasi img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;
            <span>Afiliasi</span>
            <i class="material-icons pull-right">&#xE5CC;</i>
        </a>
        <ul class="list child">
                        <li data-fitur="6003" id="Download-Brosur-Flyer" class=''>
                <a href="help/brosur"> 
                    <img class="img-sidebar-child icon icon-bb-sidebar-downld-flyer-dan-brosur" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                    <span>Download Brosur/Flyer</span>
                </a>
            </li>
                        <li data-fitur="6003" id="Download-Ebook" class=''>
                <a href="help/ebook"> 
                    <img class="img-sidebar-child icon icon-bb-sidebar-download-ebook" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                    <span>Download Ebook</span>
                </a>
            </li>
                        <li data-fitur="6003" id="Download-Panduan-Bisnis" class=''>
                <a href="help/panduan_bisnis"> 
                    <img class="img-sidebar-child icon icon-bb-sidebar-downld-panduan-bisnis" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                    <span>Download Panduan Bisnis</span>
                </a>
            </li>
                        <li data-fitur="1009" id="Pengunjung-Afiliasi" class=''>
                <a href="account/statistic"> 
                    <img class="img-sidebar-child icon icon-bb-sidebar-statistic-icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAgAAAAIAAQMAAADOtka5AAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAADZJREFUeNrtwQEBAAAAgiD/r25IQAEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAfBuCAAABY3VQpAAAAABJRU5ErkJggg=="/>&nbsp;
                    <span>Pengunjung Afiliasi</span>
                </a>
            </li>
                    </ul>
    </li>-->
        
    
        <li id="menu-deposit">
<!--            <a class='' href="/pages/deposit">
                <img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Deposti-TOP-UP-saldo.png"/>&nbsp;
                <span>Deposit</span>
            </a>-->
            <a class='' href="/pages/deposit">
                <img class="icon-bb-sidebar-deposti-top-up-saldo img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;
                <span>Deposit</span>
            </a>
        </li>
                <li id="Belanja">
<!--            <a class='' href="penjualan/supplier">
                <img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Pembelian.png"/>&nbsp;
                <span>Belanja</span>
            </a>-->
            <a class='' href="penjualan/supplier">
                <img class="icon-bb-sidebar-pembelian img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;
                <span>Belanja</span>
            </a>
        </li>
                
        <li id="Edukasi">
        <a href="javascript:void(0)">
            <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Edukasi.png"/>&nbsp;-->
            <img class="icon-bb-sidebar-edukasi img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;
            <span>Edukasi</span>
            <i class="material-icons pull-right">&#xE5CC;</i>
        </a>

        <ul class="list child">
                            <li data-fitur="6004" id="Pembelian-Buku" class=''>
                    <a href="pages/penjualan_buku">
                        <img class="img-sidebar-child icon icon-bb-sidebar-pembelian-buku" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Pembelian Buku</span>
                    </a>
                </li>
                            <li data-fitur="6005" id="Pembelian-Seminar" class=''>
                    <a href="/pages/penjualan_seminar">
                        <img class="img-sidebar-child icon icon-bb-sidebar-pembelian-seminar" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Pembelian Seminar</span>
                    </a>
                </li>
              
        </ul>
    </li>
            <li id="Pembayaran-Tagihan">
        <a href="javascript:void(0)">
            <img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Pembayaran-Tagihan.png"/>&nbsp;<span>Pembayaran Tagihan</span>
            <i class="material-icons pull-right">&#xE5CC;</i>
        </a>
        <ul class="list child">
                            <li data-fitur="4022" id="Pulsa-dan-Paket-Data" class=''>
                    <a href="pages/pulsa"> 
                        <img class="icon-bb-sidebar-pulsa img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII"/>&nbsp;
                        <span>Pulsa dan Paket Data</span>
                    </a>
                </li>
                            <li data-fitur="4017" id="Listrik" class=''>
                    <a href="pages/electric"> 
                        <img class="icon-bb-sidebar-listrik img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Listrik</span>
                    </a>
                </li>
                            <li data-fitur="4011" id="Air" class=''>
                    <a href="pages/water"> 
                        <img class="icon-bb-sidebar-pdam img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Air</span>
                    </a>
                </li>
                            <li data-fitur="4022" id="Voucher-Game" class=''>
                    <a href="pages/vouchergame"> 
                        <img class="icon-bb-sidebar-voucher-game img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Voucher Game</span>
                    </a>
                </li>
                            <li data-fitur="4029" id="TV-Berlangganan" class=''>
                    <a href="pages/tvcable"> 
                        <img class="icon-bb-sidebar-tv-kabelo img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>TV Berlangganan</span>
                    </a>
                </li>
                            <li data-fitur="4046" id="BPJS-KS" class=''>
                    <a href="pages/bpjsks?p=ASRBPJSKS"> 
                        <img class="icon-bb-sidebar-bpjs img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>BPJS KS</span>
                    </a>
                </li>
                            <li data-fitur="4025" id="Speedy" class=''>
                    <a href="pages/speedy"> 
                        <img class="icon-bb-sidebar-speedy img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Speedy</span>
                    </a>
                </li>
                            <li data-fitur="4005" id="Kartu-Kredit" class=''>
                    <a href="pages/creditcard"> 
                        <img class="icon-bb-sidebar-kartu-kredit img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Kartu Kredit</span>
                    </a>
                </li>
                            <li data-fitur="4023" id="Telepon-Flexi" class=''>
                    <a href="pages/phone"> 
                        <img class="icon-bb-sidebar-telephone img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Telepon/Flexi</span>
                    </a>
                </li>
                            <li data-fitur="4001" id="Asuransi" class=''>
                    <a href="pages/insurance"> 
                        <img class="icon-bb-sidebar-asuransi img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Asuransi</span>
                    </a>
                </li>
                            <li data-fitur="4027" id="Hp-Pasca-Bayar" class=''>
                    <a href="pages/pulsapasca"> 
                        <img class="icon-bb-sidebar-hp-pasca-bayar img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Hp Pasca Bayar</span>
                    </a>
                </li>
                            <li data-fitur="4007" id="Cicilan" class=''>
                    <a href="pages/multifinance"> 
                        <img class="icon-bb-sidebar-cicilan img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Cicilan</span>
                    </a>
                </li>
                            <li data-fitur="4047" id="Zakat" class=''>
                    <a href="pages/zakat"> 
                        <img class="icon-bb-sidebar-zakat img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Zakat</span>
                    </a>
                </li>
                            <li data-fitur="4014" id="Bayar-Pajak" class=''>
                    <a href="pages/pajak"> 
                        <img class="icon-bb-sidebar-pkb img-sidebar-child icon" src="data:image/png;base64,"/>&nbsp;
                        <span>Bayar Pajak</span>
                    </a>
                </li>
              
        </ul>
    </li>
            <li id="Tiket-dan-Reservasi">
        <a href="javascript:void(0)">
            <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Ticket-&-Reservasi.png"/>&nbsp;<span>Tiket dan Reservasi</span>-->
            <img class="img-sidebar icon icon-bb-sidebar-ticket-reservasi" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;<span>Tiket dan Reservasi</span>
            <i class="material-icons pull-right">&#xE5CC;</i>
        </a>
        <ul class="list child">
                            <li data-fitur="4019" id="Pesawat-Terbang" class='' title="">
                    <a href="pages/airplane"> 
                        <img class="icon-bb-sidebar-pesawat img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Pesawat Terbang</span>
                    </a>
                </li>
                            <li data-fitur="4003" id="Kereta-Api" class='' title="">
                    <a href="pages/train"> 
                        <img class="icon-bb-sidebar-kereta-api img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Kereta Api</span>
                    </a>
                </li>
                            <li data-fitur="4003" id="Railink" class='' title="">
                    <a href="pages/train?utm_source=railink"> 
                        <img class="icon-bb-sidebar-railink img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Railink</span>
                    </a>
                </li>
                            <li data-fitur="4019" id="Cek-Kode-Booking-Pesawat" class='' title="Cek Kode Booking Pesawat">
                    <a href="pages/book"> 
                        <img class="icon-bb-sidebar-kode-booking-pesawat img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Cek Kode Booking Pesawat</span>
                    </a>
                </li>
                            <li data-fitur="4019" id="Cek-Status-Refund" class='' title="">
                    <a href="pages/list_refund"> 
                        <img class="icon-bb-sidebar-cek-status-refund img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Cek Status Refund</span>
                    </a>
                </li>
              
        </ul>
    </li>
            <li id="Transaksi-Keuangan">
        <a href="javascript:void(0)">
            <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Transaksi-Keuangan.png"/>&nbsp;<span>Transaksi Keuangan</span>-->
            <img class="icon-bb-sidebar-transaksi-keuangan img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;<span>Transaksi Keuangan</span>
            <i class="material-icons pull-right">&#xE5CC;</i>
        </a>
        <ul class="list child">
                            <li data-fitur="3011" id="Transfer-Antar-ID-BebasBayar" class=''>
                    <a href="pages/scti"> 
                        <img class="icon-bb-sidebar-transfer-id-bb img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Transfer Antar ID BebasBayar</span>
                    </a>
                </li>
                            <li data-fitur="3005" id="Transfer-Saldo-Ke-Bank" class=''>
                    <a href="pages/cashout_bank"> 
                        <img class="icon-bb-sidebar-transfer-saldo-ke-bank img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Transfer Saldo Ke Bank</span>
                    </a>
                </li>
                            <li data-fitur="4051" id="Keuangan-Lain-Lain" class='menu-disabled'>
                    <a href="pages/cashout_misc"> 
                        <img class="icon-bb-sidebar-keuangan-lain-lain img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Keuangan Lain Lain</span>
                    </a>
                </li>
                    </ul>
    </li>
                <li data-fitur="1022" id="Pembayaran-Otomatis"  class=''>
            <a href="pages/calendar">
                <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Pembayaran-Otomtis.png"/>&nbsp;<span>Pembayaran Otomatis</span>-->
                <img class="icon-bb-sidebar-pembayaran-otomtis img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;<span>Pembayaran Otomatis</span>
                <small class="label pull-right" id='unread-calendar'></small>
            </a>
        </li>
            
        <li id="Laporan">
        <a href="javascript:void(0)">
            <!--<img class="icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Laporan.png"/>&nbsp;<span>Laporan</span>-->
            <img class="img-sidebar icon icon-bb-sidebar-laporan" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;<span>Laporan</span>
            <i class="material-icons pull-right">&#xE5CC;</i>
        </a>
        <ul class="list child">
                            <li data-fitur="103" id="Laporan-Transaksi" 
                    class=''>
                    <a href="pages/report/payment"> 
                        <img class="icon-bb-sidebar-laporan-transaksi img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Laporan Transaksi</span>
                    </a>
                </li>
                            <li data-fitur="103" id="Mutasi-Saldo" 
                    class=''>
                    <a href="pages/report/mutation"> 
                        <img class="icon-bb-sidebar-mutasi-saldo img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Mutasi Saldo</span>
                    </a>
                </li>
                            <li data-fitur="103" id="Cashback-Transaksi" 
                    class=''>
                    <a href="pages/report/bonus_komisi_trx"> 
                        <img class="icon-bb-sidebar-cashback-transaksi img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Cashback Transaksi</span>
                    </a>
                </li>
                            <li data-fitur="103" id="Riwayat-Pembelian" 
                    class=''>
                    <a href="/pages/riwayat_pembelian"> 
                        <img class="icon-bb-sidebar-riwayat-pembelian img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Riwayat Pembelian</span>
                    </a>
                </li>
              
        </ul>
    </li>    
        
        <li id="Administrasi">
        <a href="javascript:void(0)">
            <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Akun-Saya.png"/>&nbsp;<span>Akun Saya</span>-->
            <img class="img-sidebar icon icon-bb-sidebar-akun-saya" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;<span>Akun Saya</span>
            <i class="material-icons pull-right">&#xE5CC;</i>
        </a>
        <ul class="list child">
                            <li data-fitur="1005" id="Detail-Akun" class=''>
                    <a href="account/detail"> 
                        <img class="icon-bb-sidebar-detail-akun img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Detail Akun</span>
                    </a>
                </li>
                            <li data-fitur="1001" id="Ganti-PIN" class=''>
                    <a href="account/pin"> 
                        <img class="icon-bb-sidebar-ganti-pin img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Ganti PIN</span>
                    </a>
                </li>
                            <li data-fitur="1001" id="Upgrade-Akun" class=''>
                    <a href="account/upgrade"> 
                        <img class="icon-bb-sidebar-akun-saya img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Upgrade Akun</span>
                    </a>
                </li>
                            <li data-fitur="103" id="Ringkasan-Akun" class=''>
                    <a href="account/summary"> 
                        <img class="icon-bb-sidebar-ringkasan-akun img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Ringkasan Akun</span>
                    </a>
                </li>
                            <li data-fitur="6001" id="Poin" class=''>
                    <a href="account/reward_point"> 
                        <img class="icon-bb-sidebar-points-icon img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEAAAABAAQMAAACQp+OdAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAAA9JREFUeNpjYBgFo4B8AAACQAAB4E0dRAAAAABJRU5ErkJggg=="/>&nbsp;
                        <span>Poin</span>
                    </a>
                </li>
                            <li data-fitur="6001" id="Perangkat-Saya" class=''>
                    <a href="account/devices"> 
                        <img class="icon-bb-sidebar-device img-sidebar-child icon" src="data:image/svg+xml;utf8;base64,"/>&nbsp;
                        <span>Perangkat Saya</span>
                    </a>
                </li>
                            <li data-fitur="6001" id="QR-Code" class=''>
                    <a href="account/my_qrcode"> 
                        <img class="icon-bb-sidebar-devices-icon img-sidebar-child icon" src="data:image/png;base64,"/>&nbsp;
                        <span>QR Code</span>
                    </a>
                </li>
                            <li data-fitur="6001" id="Tukarkan-Voucher" class=''>
                    <a href="account/voucher"> 
                        <img class="icon-bb-sidebar-tukarkan-voucher img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                        <span>Tukarkan Voucher</span>
                    </a>
                </li>
              
        </ul>
    </li>
                <li id="Pesan/Kotak-Masuk">
            <a href="javascript:void(0)">
                <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Pesan-Kotak-Masuk.png"/>&nbsp;-->
                <img class="img-sidebar icon icon-bb-sidebar-pesan-kotak-masuk" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;
                <span>Pesan/Kotak Masuk &nbsp; 
                    <!--<small class="label unread-mailbox" style="background-color:red !important;padding: 2px 5px;color: #FFF;">&nbsp;</small>-->
                
                </span> 
                <i class="material-icons pull-right">&#xE5CC;</i>
                
            </a>
            <ul class="list child">
                                    <li data-fitur="1020" id="Komplain" class=''>
                        <a href="/mailbox/komplain"> 
                            <img class="icon-bb-sidebar-komplain img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                            <span>Komplain</span>
                                                            <!--<small class="unread-mailbox"></small>-->
                            
                        </a>
                    </li>
                                        <li data-fitur="1020" id="Pesan-Antar-Member" class=''>
                        <a href="/mailbox"> 
                            <img class="icon-bb-sidebar-pesan-antar-pesan img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                            <span>Pesan Antar Member</span>
                            
                        </a>
                    </li>
                      
            </ul>
        </li>
        
            <li class='treeview' id="Info-Layanan">
            <a href="/">
                <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Info-Layanan.png"/>&nbsp;-->
                <img class="img-sidebar icon icon-bb-sidebar-info-layanan" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;
                <span>Info Layanan</span>
                <i class="material-icons pull-right">&#xE5CC;</i>
            </a>
            <ul class="list child">
                                    <li data-fitur="103" id="Info-Cashback-Transaksi" class=''>
                        <a href="/pages/cashback_transaksi"> 
                            <img class="icon-bb-sidebar-info-cashback-transaksi img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                            <span>Info Cashback Transaksi</span>
                        </a>
                    </li>
                                    <li data-fitur="103" id="Info-Transaksi-via-Text" class=''>
                        <a href="/pages/list_produk"> 
                            <img class="icon-bb-sidebar-info-transaksi-via-text img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                            <span>Info Transaksi via Text</span>
                        </a>
                    </li>
                                    <li data-fitur="103" id="Info-Harga-Pulsa-Game" class=''>
                        <a href="/pages/harga_pulsa_game"> 
                            <img class="icon-bb-sidebar-info-harga-pulsa-via-game img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                            <span>Info Harga Pulsa Game</span>
                        </a>
                    </li>
                                    <li data-fitur="103" id="Info-Perolehan-Poin" class=''>
                        <a href="/pages/perolehan_poin"> 
                            <img class="icon-bb-sidebar-info-cashback-transaksi img-sidebar-child icon" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII="/>&nbsp;
                            <span>Info Perolehan Poin</span>
                        </a>
                    </li>
                  
            </ul>
        </li>
    
    <li>
        <a class='' href="help/contact">
            <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Kontak-Kami.png"/>&nbsp;<span>Kontak Kami</span>-->
           <img class="icon-bb-sidebar-kontak-kami img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;<span>Kontak Kami</span>
        </a>
    </li>
    <li>
        <a class='' href="help/information">
            <!--<img class="img-sidebar icon" src="https://m.bebasbayar.com/assets/img/sidebar/all/Pusat-Bantuan.png"/>&nbsp;<span>Pusat Bantuan</span>-->
            <img class="icon-bb-sidebar-pusat-bantuan img-sidebar icon" alt="" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADAAQMAAABoEv5EAAAAA1BMVEX///+nxBvIAAAAAXRSTlMAQObYZgAAABxJREFUeNrtwTEBAAAAwqD1T20ND6AAAAAA4NcAEsAAAcw7WmwAAAAASUVORK5CYII=">&nbsp;<span>Pusat Bantuan</span>
        </a>
    </li>
</ul>

<div class="slide-menu-shop bg-ff">
    <div class="grid">
        <div class="grid__item one-half" style="border-right: solid 1px #CCC;">
            <a onclick="location.href = 'lock'" tabindex="-1" class="button full-length" style="box-shadow:none;background: #FFF;">
                <i class=" material-icons">lock_outline</i>
            </a>
        </div>
        <div class="grid__item five-twelfths" style="width: 48.9%;">
            <a class="dropdown-item" href="{{ route('logout') }}"
                                       onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();">
                                        {{ __('Logout') }}
                                    </a>

                                    <form id="logout-form" action="{{ route('logout') }}" method="POST" style="display: none;">
                                        @csrf
                                    </form>
        </div>
    </div>
</div>
                </div>
            </div>
        </div>
    </nav>

    <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/processes.js"></script>
    <script type="text/javascript">
                    function showPinPopup() {
                        $.afui.popup({
                            title: "Masukkan PIN BebasBayar Anda",
                            message: "Pin: <input type='password' maxlength='6' class='af-ui-forms' id='ispin'>",
                            cancelText: "Tutup",
                            cancelCallback: function () {
                                window.location.href = "https://m.bebasbayar.com:443/";
                            },
                            doneText: "Verifikasi",
                            doneCallback: function () {
                                cekPin($('#ispin').val());
                                return false;
                            },
                            cancelOnly: false
                        });
                    }
                    function cekPin(userpin) {
                        if (userpin.length < 6) {
                            console.log("PIN tidak valid. Isikan minimal 6 digit");
                            return false;
                        }
                        $.ajax("submit/verify_pin", {
                            type: 'POST',
                            dataType: 'json',
                            timeout: 60000,
                            data: {'pin': userpin}
                        }).done(function (response) {
                            console.log(response.error);
                            if (!response.error) {
                                growl(response.result, 'info');
                            } else {
                                halt('Informasi', response.result, tutup);
                            }
//                            return false;
                        });
//                        return false;
                    }
                    function tutup() {
                        showPinPopup();
                    }
    </script>
    <!--script src='https://s10.histats.com/js15.js' type='text/javascript'></script>;
    <a href="http://www.histats.com" target="_blank" title="counter">
        <script type="text/javascript" >
            try {Histats.start(1,3329803,4,0,0,0,"");
            Histats.track_hits();} catch(err){};
        </script>
    </a>
    <noscript>
        <a href="http://www.histats.com" target="_blank">
            <img  src="https://sstatic1.histats.com/0.gif?3329803&101" alt="counter" border="0">
        </a>
    </noscript-->
    <script>
        (function (w, i, d, g, e, t, s) {
            w[d] = w[d] || [];
            t = i.createElement(g);
            t.async = 1;
            t.src = e;
            s = i.getElementsByTagName(g)[0];
            s.parentNode.insertBefore(t, s);
        })(window, document, '_gscq', 'script', '//widgets.getsitecontrol.com/67844/script.js');

        /*          (function(w,d,t,u,n,a,m){w['MauticTrackingObject']=n;
         w[n]=w[n]||function(){(w[n].q=w[n].q||[]).push(arguments)},a=d.createElement(t),
         m=d.getElementsByTagName(t)[0];a.async=1;a.src=u;m.parentNode.insertBefore(a,m)
         })(window,document,'script','https://news.bebasbayar.com/mtc.js','mt');
         
         mt('send', 'pageview',{
         email: "candra.sy90@gmail.com", 
         firstname: "Candra Syahputra",
         phone: "6283164306373",
         mobile: "6283164306373",
         level: "Premium",
         address2: "Jl Penegak LK 1 Kel Tanah Merah",
         idmember: "1233657",
         ismobile : "1",
         saldo : "5.666,00"
         });*/

    </script>
    <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/intro.js"></script>
    <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/show_walkthrough.js"></script>
    <script type="text/javascript" charset="utf-8" src="https://m.bebasbayar.com/assets/js/show_dynamic_content.js"></script>
</body>
</html>
